...
auto create_particles(){
  double body_length = 40. * 1e-3;
  double body_height = 200. * 1e-3;
  double body_depth = 40. * 1e-3;
  double body_spacing = 5 * 1e-3;
  std::vector<double> xf_1, yf_1, zf_1;
  std::tie(xf_1, yf_1, zf_1) = CabanaRigidBody::Geometry::create_3d_block(body_length, body_height, body_depth, body_spacing);
  std::vector<int> body_id_1(xf_1.size(), 0);

  body_length = 50. * 1e-3;
  body_height = 40. * 1e-3;
  body_depth = 40. * 1e-3;
  body_spacing = 5 * 1e-3;
  std::vector<double> xf_2, yf_2, zf_2;
  std::tie(xf_2, yf_2, zf_2) = CabanaRigidBody::Geometry::create_3d_block(body_length, body_height, body_depth, body_spacing);
  std::vector<int> body_id_2(xf_2.size(), 1);
  for (auto& v : xf_2) {
    v += 40. * 1e-3 + body_spacing;
  }

  std::vector<double> x_full = xf_1;
  x_full.insert(x_full.end(), xf_2.begin(), xf_2.end()); // Append vec2
  std::vector<double> y_full = yf_1;
  y_full.insert(y_full.end(), yf_2.begin(), yf_2.end()); // Append vec2
  std::vector<double> z_full = zf_1;
  z_full.insert(z_full.end(), zf_2.begin(), zf_2.end()); // Append vec2

  // create body id to differentiate the rigid bodies
  std::vector<int> body_id_full = body_id_1;
  body_id_full.insert(body_id_full.end(), body_id_2.begin(), body_id_2.end()); // Append vec2
...
void Problem04DzhanibekovEffectRigidBody3D()
{
  ...
  // =================================================================
  auto particles = create_particles<MemorySpace, ExecutionSpace>();
  particles.setup_rigid_body_properties();
  // CabanaRigidBody::print_rigid_body_properties(particles);
  particles.set_principal_moi_and_orientation();
  // double lin_vel[3] = {0.0, 0.0, 0.0};
  double ang_vel[6] = {0.0, 0.0, 2. * M_PI, 0.0, 0.0, - 2. * M_PI};
