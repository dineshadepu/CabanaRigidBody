...
auto create_particles(double body_length, double body_height, double body_spacing){
  std::vector<double> xf, yf;
  std::tie(xf, yf) = CabanaRigidBody::Geometry::create_2d_block(body_length, body_height, body_spacing);
  // std::vector<double> xb, yb;
  // std::tie(xb, yb) = CabanaSPH::Geometry::create_2d_dam(dam_length, dam_height, dam_spacing, dam_layers);
  ...
  auto no_bodies = 1;
  auto particles = CabanaRigidBody::Particles<MemorySpace, 2>(ExecutionSpace(), no_particles, no_bodies);
 ...
 */
void Problem01FreelyTranslatingRigidBody2D( double body_length_, double body_height_, double body_spacing_)
{
  int comm_rank = -1;
...
int main( int argc, char* argv[] )
{
  ...
  if ( argc < 4 )
    {
      ...
      std::cerr << "\nfor example: ./Problem03DamBreak2D 1. 1. 0.1\n";
      Kokkos::finalize();
      MPI_Finalize();
      return 0;
    }
  ...
  double body_spacing = std::atof( argv[3] );
  // run the problem
